...
export async function POST(req: Request) {
  try {
    const { readerId, actorEmail, startTime, endTime } = await req.json();

    if (!readerId || !actorEmail || !startTime || !endTime) {
    ...
    const rows = await prisma.booking.findMany({
      where: { readerId, actorEmail, startTime: start, endTime: end },
